define(function(require) {
	...
	function intro() {
		...
		app.stage.filters = [shaders.snow];

		if(!guyParticles) {
			guyParticles = new PIXI.extras.AnimatedSprite(utils.findFrameSequence("drag_000").map(frame => {return PIXI.Texture.fromFrame(frame);}));
			displayList.introGuyParticles.addChild(guyParticles);
			guyParticles.loop = true;
			guyParticles.gotoAndPlay(0);
		}
		new Timeline({
			onComplete: () => {
				//Check if the FPS is holding up
				if(PIXI.ticker.shared.FPS < 20) {
					//bad FPS, fade out the snow to cheer it up before showing the introGuy
					Tween.to(shaders.snow.uniforms, 2, {alpha: 0, onComplete: () => {
						app.stage.filters = [];
						Tween.fromTo(displayList.introGuy, 20, {x: (l ? 900 : 700), y: (l ? 300 : 500)}, { x: (l ? -800 : -800), y: (l ? -360 : -90)}, 0.1);
					}});
				} else {
					Tween.fromTo(displayList.introGuy, 20, {x: (l ? 900 : 700), y: (l ? 300 : 500)}, { x: (l ? -800 : -800), y: (l ? -360 : -90)}, 0.1);
				}
			...
			.to(displayList.introOverlay, 1.875, {alpha: 0, ease: "Linear.easeNone"}, 2.5)
			...
			.fromTo(displayList.introTitle, 0.25, {alpha: 0, x: (l ? 400 : 320), y: (l ? 243 : 519), rotation: -0.113}, {alpha: 1, rotation: -0.117, ease: "Linear.easeNone"}, 3.1)
			.fromTo(displayList.introTitle.scale, 0.25, {x: 1.9, y: 1.9}, {x: 1, y: 1, ease: "Linear.easeNone"}, 3.1)
			.fromTo(displayList.introShield, 0.125, {alpha: 0, x: (l ? 400 : 320), y: (l ? 300 : 576), rotation: -0.274}, {alpha: 1, rotation: 0.263, ease: "Linear.easeNone"}, 3.2)
			.fromTo(displayList.introShield.scale, 0.125, {x: 1, y: 1}, {x: 1.6, y: 1.6, ease: "Linear.easeNone"}, 3.2)
			.to(displayList.introTitle, 0.167, {rotation: 0.218, ease: "Linear.easeNone"}, 3.325)
			.to(displayList.introShield, 0.167, {rotation: -0.183, ease: "Linear.easeNone"}, 3.325)
			.to(displayList.introShield.scale, 0.167, {x: 1.4, y: 1.4, ease: "Linear.easeNone"}, 3.325)
			.to(displayList.introTitle, 0.08333, {rotation: 0, ease: "Linear.easeNone"}, 3.367)
			.to(displayList.introShield, 0.08333, {rotation: 0, ease: "Linear.easeNone"}, 3.367)
			.to(displayList.introShield.scale, 0.08333, {x: 1.6, y: 1.6, ease: "Linear.easeNone"}, 3.367)
			.to(displayList.introTitle, 0.4167, {x: (l ? 702 : 509), y: (l ? 136 : 302), ease: "Linear.easeNone"}, 3.9167)
			.to(displayList.introTitle.scale, 0.4167, {x: (l ? 0.2904 : 0.44), y: (l ? 0.2904 : 0.44), ease: "Linear.easeNone"}, 3.9167)
			.to(displayList.introShield, 0.4167, {x: (l ? 701 : 508), y: (l ? 154 : 330), ease: "Linear.easeNone"}, 3.9167)
			.to(displayList.introShield.scale, 0.4167, {x: (l ? 0.66 : 1), y: (l ? 0.66 : 1), ease: "Linear.easeNone"}, 3.9167)
			.to(displayList.titleLogo, 0, {alpha: 1, ease: "Linear.easeNone"}, 4.5)
			.to(displayList.introShield, 0, {alpha: 0, ease: "Linear.easeNone"}, 4.5)
			...
			.fromTo(displayList.mainGameTile0, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 126, y: -144, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile1, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 315, y: -144, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile2, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 507, y: -144, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile3, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 126, y: -2, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile4, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 315, y: -2, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile5, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 507, y: -2, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile6, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 126, y: 138, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile7, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 315, y: 138, ease: "Linear.easeNone"}, 4.9167)
			.fromTo(displayList.mainGameTile8, 0.5, {alpha: 0, x: 315, y: -2}, {alpha: 1, x: 507, y: 138, ease: "Linear.easeNone"}, 4.9167)
