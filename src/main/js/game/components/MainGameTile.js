define((require) => {
	...
	class MainGameTile extends ScratchSymbol {
		constructor() {
			...
			let foilBase = PIXI.Sprite.fromFrame("scratchOff");
			...
			this.winParticles = new PIXI.extras.AnimatedSprite(utils.findFrameSequence("snow00").map(frame => {return PIXI.Texture.fromFrame(frame);}));
			this.winParticles.loop = false;
			this.winParticles.alpha = 0;
			this.winParticles.anchor.set(0.5);
			this.winParticles.scale.set(0.6);
		...
		populate(value) {
			...
			if(this.value >= config.bigWinLevel1) {
				this.lose.texture = PIXI.Texture.fromFrame("scratchBaseSpecial");
				this.win.texture = PIXI.Texture.fromFrame("scratchBaseSpecialWin");
				this.valueSprite.text = ""; //Hide the number if it's the top prize
			} else {
				this.lose.texture = PIXI.Texture.fromFrame("scratchBaseLose");
				this.win.texture = PIXI.Texture.fromFrame("scratchBaseWin");
				this.valueSprite.text = nokFormat(value);
			}
		}
